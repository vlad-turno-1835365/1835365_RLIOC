...
// Aggiungi o modifica i nomi qui dentro a tuo piacimento!
const formatFriendlyName = (rawName) => {
  if (!rawName) return '';
  
  const dictionary = {
    // Sensori
    'WATER_TANK_LEVEL_LITERS': 'Water Tank (L)',
    'WATER_TANK_LEVEL_PERCENTAGE': 'Water Tank (%)',
    'AIR_QUALITY_PM25_PM25': 'PM2.5 Level',
    'AIR_QUALITY_PM25_PM1': 'PM1 Level',
    'AIR_QUALITY_PM25_PM10': 'PM10 Level',
    'POWER_BUS': 'Power Bus',
    'CORRIDOR_PRESSURE': 'Corridor Pressure',
    'SOLAR_ARRAY': 'Solar Array',
    'MARS/TELEMETRY/THERMAL_LOOP_FLOW_RATE': 'Thermal Loop Flow',
    'MARS/TELEMETRY/THERMAL_LOOP_TEMPERATURE': 'Thermal Loop Temp',
    'MARS/TELEMETRY/LIFE_SUPPORT_OXYGEN_PERCENT': 'Oxygen Level',
    'HYDROPONIC_PH_PH': 'Hydroponic pH',
    'ENTRANCE_HUMIDITY': 'Entrance Humidity',
    'GREENHOUSE_TEMPERATURE': 'Greenhouse Temp',
    'AIR_QUALITY_VOC_CO2E_PPM': 'CO2 Equivalent',
    'CO2_HALL': 'Hall CO2',
    'AIRLOCK-1': 'Airlock Cycles',
    'MARS/TELEMETRY/RADIATION_RADIATION_USV_H': 'Radiation Level',
    'POWER_CONSUMPTION': 'Power Consumption',
    'AIR_QUALITY_VOC_VOC_PPB': 'VOC Level',
    // Attuatori
  ...
  // 1. Se è nel dizionario, usa il nome bello
  if (dictionary[rawName]) return dictionary[rawName];

  // 2. Piano B (Fallback) per sensori sconosciuti:
  // Es: "MARS/TELEMETRY/NEW_SENSOR_NAME" -> "New Sensor Name"
    ...
    ws.onmessage = (event) => {
      ...
      if (message.type === 'init') {
        if (Object.keys(message.data).length > 0) {
          ...
          const initialTemp = message.data["GREENHOUSE_TEMPERATURE"]?.value || message.data["greenhouse_temperature"]?.value || 0;
        ...
        setSensors(prev => {
          ...
          if (message.data.sensor_name === 'GREENHOUSE_TEMPERATURE' || message.data.sensor_name === 'greenhouse_temperature') {
  ...
  const toggleActuator = async (actuatorName) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        alert(`Impossibile comunicare con l'attuatore: ${formatFriendlyName(actuatorName)}`);
        return;
      }
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-4 font-mono w-full overflow-x-hidden">
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4 mb-6">
        {Object.entries(sensors).map(([name, data]) => (
          <div key={name} className="bg-slate-900 p-5 rounded border border-slate-700 hover:border-slate-500 transition-colors">
            {/* QUI USIAMO IL FORMATTER PER I NOMI NELLE CARD */}
            <h3 className="text-xs text-slate-400 mb-1 uppercase tracking-wider truncate" title={name}>
              {formatFriendlyName(name)}
            </h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
        ...
        <div className="lg:col-span-1 bg-slate-900 p-5 rounded border border-slate-700 overflow-y-auto h-full">
          <h2 className="text-lg text-slate-300 mb-4 border-b border-slate-800 pb-2">MANUAL OVERRIDE</h2>
          <div className="flex flex-col gap-3">
            {Object.entries(actuators).map(([name, state]) => (
              <div key={name} className="flex justify-between items-center bg-slate-800 p-3 rounded border border-slate-700">
                {/* QUI USIAMO IL FORMATTER PER GLI ATTUATORI */}
                <span className="text-sm text-slate-300 uppercase truncate mr-2">
                  {formatFriendlyName(name)}
                </span>
                <button 
          ...
          <div className="flex-1 min-w-[200px]">
            ...
            <select className="w-full bg-slate-800 border border-slate-700 rounded p-2 text-sm focus:border-emerald-500 outline-none" 
              ...
              {Object.keys(sensors).map((sensorName) => (
                // L'utente vede il nome bello, ma React salva il `sensorName` originale nel value!
                <option key={sensorName} value={sensorName}>{formatFriendlyName(sensorName)}</option>
              ))}
            </select>
          </div>
          ...
          <div className="flex-1 min-w-[200px]">
            ...
            <select className="w-full bg-slate-800 border border-slate-700 rounded p-2 text-sm outline-none"
              ...
              {Object.keys(actuators).map((actuatorName) => (
                <option key={actuatorName} value={actuatorName}>{formatFriendlyName(actuatorName)}</option>
              ))}
            </select>
          </div>
            ...
            <tbody>
              {rules.map((r) => (
                <tr key={r.id} className="border-b border-slate-800 bg-slate-900 hover:bg-slate-800">
                  {/* ANCHE LA TABELLA ORA USA I NOMI BELLI */}
                  <td className="px-4 py-3"><span className="text-emerald-400">{formatFriendlyName(r.sensor)}</span> {r.operator} <span className="text-white">{r.value}</span></td>
                  <td className="px-4 py-3 text-slate-300">Set <span className="text-white">{formatFriendlyName(r.actuator)}</span> to <span className={r.state === 'ON' ? 'text-red-400' : 'text-slate-500'}>{r.state}</span></td>
